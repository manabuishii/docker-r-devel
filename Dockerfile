...
RUN svn checkout -r 76252 https://svn.r-project.org/R/trunk/  /usr/src/r-devel
RUN cd /usr/src/r-devel ;  ./configure --without-recommended-packages --enable-R-shlib --with-x=no --prefix=/usr/local/r-76252 ; ./tools/rsync-recommended ; ./tools/link-recommended ; make ; bin/R CMD INSTALL src/library/Recommended/MASS.tgz ; bin/R CMD INSTALL src/library/Recommended/lattice.tgz ; bin/R CMD INSTALL src/library/Recommended/Matrix.tgz
RUN cd /usr/src/r-devel ; make check
RUN cd /usr/src/r-devel ; make install
RUN ln -s /usr/local/r-76252/bin/R /usr/local/bin
