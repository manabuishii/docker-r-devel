...
RUN git clone --depth 1 https://github.com/wch/r-source.git /usr/src/r-devel ; cd /usr/src/r-devel ; git fetch --depth 1 origin 9a6f4af182c6889d53c95c89c138177ccfac379a ; git checkout FETCH_HEAD
RUN cd /usr/src/r-devel ; git svn init --prefix=origin/ -s https://svn.r-project.org/R/ ;  ./tools/rsync-recommended ; ./tools/link-recommended ;./configure --without-recommended-packages --enable-R-shlib --with-x=no --prefix=/usr/local/r-79909 ; make ; bin/R CMD INSTALL src/library/Recommended/MASS.tgz ; bin/R CMD INSTALL src/library/Recommended/lattice.tgz ; bin/R CMD INSTALL src/library/Recommended/Matrix.tgz
RUN apt-get install -y locales
RUN echo "en_US UTF-8" > /etc/locale.gen
RUN locale-gen
RUN cd /usr/src/r-devel ; make check
RUN cd /usr/src/r-devel ; make install
RUN ln -s /usr/local/r-79909/bin/R /usr/local/bin
